...
const ConsultantProfileSetup = () => {
  ...
  useEffect(() => {
    ...
    if (parsed.userType !== 'consultant' || parsed.registrationType !== 'individual') {
      navigate('/signup');
      return;
    }
  ...
  const handleComplete = async () => {
    ...
    // Create consultant account with dummy data
    const dummyUser = {
      id: `${Date.now()}`,
      name: 'Demo Consultant',
      email: credentials.email,
      role: 'consultant' as 'user' | 'consultant',
    };
    ...
    // Redirect to consultant landing page
          ...
          <Card className="border-2 border-[#E8E3DE] shadow-lg">
            <CardHeader className="text-center pb-6 border-b border-[#E8E3DE]">
              ...
              <p className="text-[#8B8680] mt-2">
                Complete your profile setup to start offering your services
              </p>
            </CardHeader>
              ...
              <div className="space-y-6">
                <div className="text-center space-y-4">
                  <h3 className="text-xl font-bold text-[#2B2520]">
                    Your Consultant Account is Ready!
                  </h3>
                  <p className="text-[#8B8680]">
                    You're all set to showcase your expertise and connect with clients.
                  </p>
                </div>
                ...
                <div className="bg-[#F9F6F3] rounded-lg p-6 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[#2B2520]">Create your professional profile</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" />
                    <span className="text-[#2B2520]">List your services and expertise</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" />
                    <span className="text-[#2B2520]">Receive project requests</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" />
                    <span className="text-[#2B2520]">Build your client base</span>
                  </div>
                </div>
